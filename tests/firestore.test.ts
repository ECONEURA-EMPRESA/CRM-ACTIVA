...
describe('Firestore Security Rules', () => {
    ...
    it('allows user to create their own patient', async () => {
        ...
        await assertSucceeds(db.collection('patients').doc('patient_1').set({
            userId: userId,
            firstName: 'Test',
            lastName: 'Patient'
        }));
    ...
    it('DENIES creating patient for another user', async () => {
        ...
        await assertFails(db.collection('patients').doc('patient_2').set({
    ...
    it('DENIES reading others patients', async () => {
        ...
        await testEnv.withSecurityRulesDisabled(async (context) => {
            await context.firestore().collection('patients').doc('patient_victim').set({ userId: victimId });
        });
