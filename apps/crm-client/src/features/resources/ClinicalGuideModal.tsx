...
export const ClinicalGuideModal: React.FC<ClinicalGuideModalProps> = ({ isOpen, onClose, pathologyType }) => {
            ...
            <div className="bg-white rounded-3xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh]">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-slate-50 to-white">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-emerald-100 text-emerald-600 rounded-xl flex items-center justify-center"><BookOpen size={20} /></div>
                        <div>
                            <h2 className="text-xl font-black text-slate-800">Guía Clínica</h2>
                            <p className="text-sm text-slate-500 font-medium">Protocolo Sugerido: {guide.title}</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <ul className="space-y-2">
                            {guide.objectives.map((o: string, i: number) => (
                                <li key={i} className="flex items-start gap-2 text-sm text-slate-600 bg-blue-50/50 p-3 rounded-xl border border-blue-100">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {guide.techniques.map((t: string, i: number) => (
                                <div key={i} className="p-3 bg-amber-50 border border-amber-100 rounded-xl text-sm font-bold text-amber-800 text-center">
                                    {t}
                                </div>
                            ))}
                        </div>
