    ...
    return (
        <div className="max-w-5xl mx-auto animate-in fade-in space-y-8">
            <header>
                ...
                <p className="text-slate-500 mt-1">Recursos clínicos, guías y protocolos oficiales del Método Activa.</p>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {Object.entries(CLINICAL_GUIDES).map(([key, guide]) => (
                            <Card key={key} className="p-6 hover:shadow-lg transition-all group cursor-pointer border-t-4 border-indigo-500">
                                <h4 className="font-bold text-slate-900 mb-2 group-hover:text-indigo-600 transition-colors">{guide.title}</h4>
                                <div className="space-y-2">
                                    {guide.techniques.slice(0, 3).map((t, i) => (
                                        <div key={i} className="text-xs text-slate-500 bg-slate-50 px-2 py-1 rounded border border-slate-100 truncate">
                                            {t}
                                        </div>
                                    ))}
                                    {guide.techniques.length > 3 && <div className="text-[10px] text-indigo-500 font-bold px-1">+{guide.techniques.length - 3} más</div>}
                                </div>
                            </Card>
                        ))}
                    </div>
                ...
                <Card className="p-6">
                    ...
                    <div className="space-y-3">
                        {['Consentimiento Informado v2024.pdf', 'Hoja de Registro de Sesión.pdf', 'Escala MOCA (Vacía).pdf', 'Cuestionario de Ingreso.pdf'].map((file, i) => (
                            <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 hover:bg-pink-50 transition-colors group cursor-pointer">
                ...
                <Card className="p-6">
                    ...
                    <div className="space-y-3">
                        {[
                            ...
                            <a key={i} href={link.url} className="block p-3 rounded-xl bg-slate-50 hover:bg-emerald-50 transition-colors group">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-medium text-slate-700 group-hover:text-emerald-800">{link.title}</span>
