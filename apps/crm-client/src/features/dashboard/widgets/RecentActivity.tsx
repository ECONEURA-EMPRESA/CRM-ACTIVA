...
export const RecentActivity: React.FC<RecentActivityProps> = ({ patients, onNavigate }) => {
    ...
    const getPhaseForSessionIndex = (index: number) => {
        ...
        const count = index + 1;
        return TREATMENT_PHASES.find(p => {
            const [min, max] = p.range.split('-').map(Number);
            return count >= min && count <= max;
        })?.id || 1;
    };

    const recentSessions: RecentSession[] = patients.flatMap(p => {
                    ...
                    >
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-xs text-slate-500 mt-0.5">
                                {s.date} • {s.isAbsent ? <span className="text-red-500 font-bold bg-red-50 px-1.5 py-0.5 rounded">Ausencia</span> : (s.computedPhase ? `Fase ${s.computedPhase}` : 'Sesión')}
                            </p>
                        </div>
