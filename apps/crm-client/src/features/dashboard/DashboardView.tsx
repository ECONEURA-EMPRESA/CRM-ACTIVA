...
import {
    ...
    ChevronRight, Zap, PlusCircle, Search,
    Settings, UserCheck, FileText, CalendarDays
} from 'lucide-react';
...
export const DashboardView: React.FC<DashboardViewProps> = ({ patients, onViewChange }) => {
    ...
    const recentSessions = patients.flatMap(p => {
        ...
        <div className="space-y-8 animate-in fade-in max-w-7xl mx-auto">
            <header className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-4 w-full md:w-auto items-center">
                    <button onClick={() => login(role === 'admin' ? 'therapist' : 'admin')} className="flex items-center gap-2 px-3 py-1 bg-slate-200 rounded-full text-xs font-bold text-slate-700 hover:bg-slate-300 transition-colors">
                        <Shield size={12} /> {role === 'admin' ? 'MODO ADMIN' : 'MODO TERAPEUTA'}
                    </button>
                    {role === 'admin' && (
                        <>
                            <Button variant="secondary" size="md" icon={Download} onClick={() => downloadPatientsCSV(patients)}>Exp. DB</Button>
                        </>
                    )}
                    <div className="relative flex-1 md:flex-none"><Search className="absolute left-3 top-3 text-slate-400" size={18} /><input className="input-pro pl-10 py-3" placeholder="Buscar paciente..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} /></div>
                    <Button icon={PlusCircle} onClick={onNewPatient} size="md" className="py-3 px-6 shadow-lg">Nueva Admisión</Button>
                </div>
            </header>
                ...
                <Card className="p-6 h-full flex flex-col">
                    ...
                    <div className="space-y-3 flex-1">
                        {recentSessions.map((s, i) => (
                            <div key={i} className="flex items-center gap-4 p-4 hover:bg-slate-50 rounded-xl transition-all border border-transparent hover:border-slate-100 cursor-pointer group" onClick={() => { /* Need to handle navigation to detail with patient set */ /* Simple hack: tell parent to go to patients then select? Or just ignore for now as 'sessions' view is safer */ onNavigate('sessions'); }}>
                                ...
                                <div className="flex-1"><h4 className="font-bold text-sm text-slate-900">{s.patientName}</h4><p className="text-xs text-slate-500 mt-0.5">{s.date} • {s.isAbsent ? <span className="text-red-500 font-bold bg-red-50 px-1.5 py-0.5 rounded">Ausencia</span> : (s.computedPhase ? `Fase ${s.computedPhase}` : 'Sesión')}</p></div>
