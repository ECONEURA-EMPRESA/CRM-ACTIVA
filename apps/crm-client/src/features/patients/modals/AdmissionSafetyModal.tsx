...
import {
    ShieldAlert, AlertTriangle, Music, HeartPulse,
    BrainCircuit, CheckCircle2, X
} from 'lucide-react';
import { ClinicalSafetyProfile, MusicalIdentity, PsychosocialContext } from '../../../lib/types';

interface AdmissionSafetyModalProps {
    onClose: () => void;
    onSave: (data: { safety: ClinicalSafetyProfile, iso: MusicalIdentity, ctx: PsychosocialContext }) => void;
    initialData?: any;
    isChild?: boolean;
}

export const AdmissionSafetyModal: React.FC<AdmissionSafetyModalProps> = ({ onClose, onSave, initialData, isChild = false }) => {

    // 1. SAFETY (SEMÁFORO ROJO)
    const [safety, setSafety] = useState<ClinicalSafetyProfile>(initialData?.safetyProfile || {
        epilepsy: false,
        dysphagia: false,
        flightRisk: false,
    ...
    const [iso, setIso] = useState<MusicalIdentity>(initialData?.musicalIdentity || {
        likes: [],
        dislikes: [],
        biographicalSongs: [],
    ...
    // 3. CONTEXT
    const [ctx, setCtx] = useState<PsychosocialContext>(initialData?.socialContext || {
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-2 md:p-4">
            <div className="bg-white w-full max-w-4xl rounded-xl md:rounded-3xl shadow-2xl max-h-[95vh] md:max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in-95">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                            <div className="bg-red-50 p-4 md:p-6 rounded-2xl border border-red-100">
                                ...
                                <TagInput
                                    ...
                                    onRemove={(t) => setIso(p => ({ ...p, dislikes: p.dislikes.filter(x => x !== t) }))}
                                    color="red"
                                />
                            </div>
                            ...
                            <div className="bg-indigo-50 p-4 md:p-6 rounded-2xl border border-indigo-100">
                                ...
                                <TagInput
                                    ...
                                    onAdd={(t) => setIso(p => ({ ...p, biographicalSongs: [...p.biographicalSongs, t] }))}
                                    onRemove={(t) => setIso(p => ({ ...p, biographicalSongs: p.biographicalSongs.filter(x => x !== t) }))}
                                    color="indigo"
                                />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <TagInput
                                    ...
                                    onAdd={(t) => setCtx(c => ({ ...c, recentLifeEvents: [...c.recentLifeEvents, t] }))}
                                    onRemove={(t) => setCtx(c => ({ ...c, recentLifeEvents: c.recentLifeEvents.filter(x => x !== t) }))}
                                    color="emerald"
                                />
                            </div>
                        </div>
                ...
                <div className="p-4 md:p-6 border-t border-slate-100 bg-slate-50 flex flex-col-reverse md:flex-row justify-end gap-3 shrink-0">
                    <button onClick={onClose} className="btn-ghost w-full md:w-auto justify-center">Cancelar</button>
                    <button onClick={() => onSave({ safety, iso, ctx })} className="btn-primary w-full md:w-auto justify-center flex items-center gap-2 shadow-xl shadow-indigo-200">
...
const RiskToggle = ({ label, desc, active, onClick }: any) => (
    ...
    >
        ...
        <p className={`text-[10px] md:text-xs mt-1 leading-tight ${active ? 'text-red-100' : 'text-slate-400'}`}>{desc}</p>
    </div>
);
        ...
        <div className="w-full">
            <div className="flex flex-wrap gap-2 mb-2">
                {tags.map((t: string, i: number) => (
                    <span key={i} className={`px-2 py-1 rounded bg-${color}-100 text-${color}-700 text-xs font-bold flex items-center gap-1`}>
                        {t} <button onClick={() => onRemove(t)} className="hover:text-red-600"><X size={12} /></button>
                    </span>
                ))}
            </div>
            <input
