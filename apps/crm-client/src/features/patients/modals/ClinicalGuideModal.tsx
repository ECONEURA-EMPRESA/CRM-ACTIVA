...
export const ClinicalGuideModal: React.FC<ClinicalGuideModalProps> = ({ onClose }) => {
    ...
    const tabs = [
        ...
        { id: 'mood', label: 'Salud Mental', icon: Heart, color: 'text-emerald-600', bg: 'bg-emerald-50' },
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-4xl rounded-2xl shadow-3d overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
                    <h2 className="text-xl font-bold flex items-center gap-3 text-slate-800"><BookOpen className="text-pink-600" /> Guías Clínicas Básicas</h2>
                    ...
                    <div className="prose prose-slate max-w-none">
                        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm mb-6">
                            <h3 className="text-2xl font-black text-slate-800 mb-6 flex items-center gap-3">
                                {guide.title}
                                <span className="text-xs font-bold bg-slate-100 text-slate-500 px-3 py-1 rounded-full uppercase tracking-widest">Protocolo MT</span>
                            </h3>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <h4 className="font-bold text-pink-600 mb-4 border-b border-pink-100 pb-2 uppercase text-sm tracking-wide">Objetivos Terapéuticos</h4>
                                ...
                                <div>
                                    <h4 className="font-bold text-blue-600 mb-4 border-b border-blue-100 pb-2 uppercase text-sm tracking-wide">Técnicas Sugeridas</h4>
                                    <ul className="space-y-3">
                            ...
                            <div className="mt-8 bg-amber-50 p-6 rounded-xl border border-amber-100">
                                <h4 className="font-bold text-amber-700 mb-3 flex items-center gap-2 uppercase text-sm tracking-wide">
                                    <div className="w-2 h-2 bg-amber-500 rounded-full animate-pulse"></div> Precauciones y Contraindicaciones
                                </h4>
                                <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    {guide.precautions.map((p, i) => (
                                        <li key={i} className="text-sm font-medium text-amber-900/80 flex items-center gap-2">
