...
export const AdmissionChecklistModal: React.FC<AdmissionChecklistModalProps> = ({ onClose, onSave, initialData, isChild = false }) => {
    ...
    const safetyItems = isChild ? (ADMISSION_CHECKS.child_safety || []) : (ADMISSION_CHECKS.adult_safety || []);
    const prepItems = isChild ? (ADMISSION_CHECKS.child_prep || []) : (ADMISSION_CHECKS.adult_prep || []);

    const toggle = (type: 'safety' | 'prep', item: string) => setChecks(prev => {
        const list = prev[type] || [];
        return { ...prev, [type]: list.includes(item) ? list.filter(i => i !== item) : [...list, item] };
    });
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl p-6 max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-slate-500 hover:text-red-500 transition-colors"><X size={20} /></button>
                </div>
                        ...
                        <div className="space-y-3">
                            {safetyItems.map(i => (
                                <label key={i} className="flex gap-3 items-start cursor-pointer group">
                                    ...
                                    <span className={`text-sm transition-colors ${checks.safety.includes(i) ? 'text-red-800 font-medium' : 'text-slate-700 group-hover:text-slate-900'}`}>
                                        {i}
                                    </span>
                                </label>
                            ))}
                        ...
                        <div className="space-y-3">
                            {prepItems.map(i => (
                                <label key={i} className="flex gap-3 items-start cursor-pointer group">
                                    ...
                                    <span className={`text-sm transition-colors ${checks.prep.includes(i) ? 'text-emerald-800 font-medium' : 'text-slate-700 group-hover:text-slate-900'}`}>
                                        {i}
                                    </span>
                                </label>
                            ))}
                ...
                <div className="mt-8 flex justify-end gap-3 pt-6 border-t border-slate-100">
                    <button onClick={onClose} className="px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors">
                        Cancelar
                    </button>
                    <button
                        onClick={() => { onSave(checks); onClose(); }}
