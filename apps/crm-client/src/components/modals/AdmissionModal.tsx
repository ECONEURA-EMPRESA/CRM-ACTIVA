...
import { UserCheck, Hash, Users, Music, Printer, Save, X } from 'lucide-react';
import { Button } from '../ui/Button';
import { printConsent } from '../../lib/utils';
import { COMMON_PATHOLOGIES } from '../../lib/constants';
...
export const AdmissionModal = ({ onClose, onSave, initialData }: AdmissionModalProps) => {
    ...
    useEffect(() => {
        if (!isManualRef && name && date) {
            const initials = name.split(' ').map((n: string) => n[0]).filter((c: string) => c).join('').toUpperCase().substring(0, 3);
    ...
    const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => { const f = e.target.files?.[0]; if (f) { const r = new FileReader(); r.onloadend = () => setPhotoPreview(r.result as string); r.readAsDataURL(f); } };
    const handleDiagnosisChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const val = e.target.value; setDiagnosisType(val);
        const pathMap: Record<string, string> = { 'Alzheimer y otras Demencias': 'dementia', 'Deterioro Cognitivo Leve': 'dementia', 'Parkinson': 'neuro', 'Daño Cerebral Adquirido': 'neuro', 'Esclerosis Múltiple / ELA': 'neuro', 'Depresión y Trastornos del Ánimo': 'mood', 'Ansiedad y Estrés': 'mood', 'TEA': 'neuro', 'TDAH': 'neuro', 'Parálisis Cerebral': 'neuro' };
