...
export const InvoiceGenerator: React.FC<InvoiceGeneratorProps> = ({ data, onClose, clinicSettings }) => {
    const [invoiceNumber, setInvoiceNumber] = useState(data.invoiceNumber || `FACT-${Date.now().toString().slice(-6)}`);
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-slate-100 animate-in fade-in duration-200">
            ...
            <div className="bg-slate-900 text-white p-4 flex justify-between items-center shadow-md no-print">
                <div className="flex items-center gap-4">
                    <button onClick={onClose} className="hover:bg-slate-800 p-2 rounded-lg transition-colors"><ArrowLeft /></button>
                ...
                <div className="flex gap-3">
                    ...
                    <Button onClick={handlePrint} variant="primary" icon={Printer} className="shadow-none ring-2 ring-offset-2 ring-offset-slate-900 ring-pink-500">IMPRIMIR / PDF</Button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 flex justify-center invoice-preview-container">
                <div className="invoice-paper bg-white w-full max-w-[210mm] min-h-[297mm] p-[20mm] shadow-2xl relative text-slate-800 print-only">
                    ...
                    <div className="flex justify-between items-start mb-16 border-b-2 border-pink-500 pb-8">
                        <div>
                            <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-2">{clinicSettings.name || 'NOMBRE CLÍNICA'}</h1>
                            <div className="text-sm text-slate-500 space-y-1">
                                <p>{clinicSettings.address || 'Dirección de la clínica'}</p>
                                <p>CIF: {clinicSettings.cif || 'B-12345678'}</p>
                                <p>{clinicSettings.phone || ''} • {clinicSettings.email || ''}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <h2 className="text-xl font-bold text-pink-600 uppercase tracking-widest mb-1">FACTURA</h2>
                    ...
                    <table className="w-full mb-12">
                        <thead>
                            <tr className="border-b border-slate-200">
                                <th className="text-left py-3 text-xs font-bold text-slate-400 uppercase">Concepto</th>
                                <th className="text-left py-3 text-xs font-bold text-slate-400 uppercase">Fecha</th>
                                <th className="text-right py-3 text-xs font-bold text-slate-400 uppercase">Importe</th>
                            </tr>
                        </thead>
                            ...
                            {data.sessions.map((s: any, i: number) => (
                                <tr key={i}>
                                    <td className="py-4 font-medium text-slate-700">
                                        {s.type === 'group' ? `Sesión Grupal: ${s.location || 'Sala Polivalente'}` : 'Sesión de Musicoterapia'}
                                        {s.notes && <div className="text-xs text-slate-400 mt-0.5 font-normal truncate max-w-md">{s.notes}</div>}
                                    </td>
                        ...
                        <tfoot>
                            <tr>
                                <td colSpan={2} className="pt-6 text-right text-sm font-bold text-slate-500 uppercase">Total a Pagar</td>
                                <td className="pt-6 text-right text-2xl font-black text-pink-600">{total.toFixed(2)} €</td>
                            </tr>
                        </tfoot>
                    </table>
                    ...
                    <div className="absolute bottom-[20mm] left-[20mm] right-[20mm] border-t border-slate-200 pt-6 text-center">
                        <p className="text-[10px] text-slate-400 leading-relaxed max-w-2xl mx-auto">
                            {clinicSettings.legalText || 'Documento informativo. El pago debe realizarse según las condiciones acordadas. De acuerdo con la Ley de Protección de Datos, sus datos son tratados con confidencialidad.'}
                        </p>
                    </div>
                </div>
            </div>
